...
IF %ERRORLEVEL% NEQ 0 echo "Failing build 2" & tasklist & call %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1
...
IF %ERRORLEVEL% NEQ 0 echo "Failing build 3" & tasklist & call %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1
...
rem SHUTDOWN ANY PREVIOUS BUILD REMNANTS
tasklist & call %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown
echo "Finished build"
