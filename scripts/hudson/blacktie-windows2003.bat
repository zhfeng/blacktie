& tasklist & %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1set NOPAUSE=true

rem SHUTDOWN ANY PREVIOUS BUILD REMNANTS
rem if exist jboss-as-7.1.0.Final call jboss-as-7.1.0.Final\bin\shutdown.bat -s %JBOSSAS_IP_ADDR%:1099 -S && cd .
...
IF %ERRORLEVEL% NEQ 0 echo "Failing build 2" & tasklist & %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1
...
IF %ERRORLEVEL% NEQ 0 echo "Failing build 3" & tasklist & %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1
...
IF %ERRORLEVEL% NEQ 0 echo "Failing build 4" & tasklist & %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1
...
dir setenv.bat
call setenv.bat
IF %ERRORLEVEL% NEQ 0 echo "Failing build 5 with error %ERRORLEVEL%" & tasklist & %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1
...
IF %ERRORLEVEL% NEQ 0 echo "Failing build 6 with error %ERRORLEVEL%" & tasklist & %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1
call run_all_quickstarts.bat tx
IF %ERRORLEVEL% NEQ 0 echo "Failing build 7 with error %ERRORLEVEL%" & tasklist & %WORKSPACE%\jboss-as-7.1.0.Final\bin\jboss-cli.bat --connect command=:shutdown & exit -1

rem SHUTDOWN ANY PREVIOUS BUILD REMNANTS
tasklist
rem call %WORKSPACE%\jboss-as-7.1.0.Final\bin\shutdown.bat -s %JBOSSAS_IP_ADDR%:1099 -S && cd .
